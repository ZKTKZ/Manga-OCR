...
void detectLetters(cv::Mat img, std::string img_name, tesseract::TessBaseAPI *api)
{
        ...
        //cv::imshow("ROI_resized", ROI_resized); cv::waitKey(0);
        ...
        for (int i = 0; i < keypoints.size(); i++){
                ...
                cv::Mat ROI_blurred, ROI_usm, ROI_resized;
                
                cv::GaussianBlur(ROI, ROI_blurred, cv::Size(9, 9), 0, 0, cv::BORDER_DEFAULT);
                ROI_usm = 4 * ROI - 2 * ROI_blurred;
                cv::resize(ROI_usm, ROI_resized, cv::Size(), 400.0/ROI.cols, 400.0/ROI.rows);
                
                api->SetImage((uchar*)ROI_resized.data, ROI_resized.size().width, ROI_resized.size().height, ROI_resized.channels(), ROI_resized.step1());
